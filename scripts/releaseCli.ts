...
async function prepareCoreRelease() {
  try {
    ...
    runCommand(`pnpm --filter dragifyjs-cli exec npm version ${versionCmd} --no-git-tag-version --no-commit-hooks`);
    ...
    runCommand('git add .');
    runCommand(`git commit -m "Release dragifyJS cli ${releaseTag}: v${newVersion}"`);

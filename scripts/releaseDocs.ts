...
async function prepareCoreRelease() {
  try {
    ...
    runCommand(`pnpm --filter @dragifyjs/docs exec npm version patch --no-git-tag-version --no-commit-hooks`);
    ...
    runCommand('git add .');
    runCommand(`git commit -m "Release dragifyJS docs: v${newVersion}"`);
